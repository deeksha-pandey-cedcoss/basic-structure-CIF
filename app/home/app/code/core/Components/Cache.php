...
class Cache extends Base implements \Psr\SimpleCache\CacheInterface 
{
    ...
    public function setDi(\Phalcon\Di\DiInterface $di):void
    {
        ...
        if( !$this->cache ){
            ...
            // todo replace it with igbinary serializer of redis
            $redisConfig['defaultSerializer'] = 'Php';
            $adapter = new Redis($serializerFactory, $redisConfig);
            $cache = new PhalconCache($adapter); // phalcon cache adapter provides necessart methods to provide PSR-16 methods
            $this->cache = $cache;
    ...
    {
        return $this->cache->getAdapter()->keys('*');
    }
}
