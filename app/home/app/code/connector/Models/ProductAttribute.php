...
class ProductAttribute extends Base
{
    ...
    public function getProductForm()
    {
        ...
        $container_field = ['id' => 1, 'group' => 'Basic Product Details'];
        ...
        $variant_fields = [];
        foreach ($variantColumns as $variantColumn_value) {
            if (in_array($variantColumn_value, ['id', 'product_id', 'source_variant_id', 'created_at', 'updated_at'])) {
                continue;
            } else {
                $label = ucwords(str_replace('_', ' ', $variantColumn_value));
                switch ($variantColumn_value) {
                    case 'mpn':
                    case 'sku':
                    case 'asin':
                        $variant_fields[] = [
                            'key' => $variantColumn_value,
                            'value' => '',
                            'title' => $label,
                            'required_group' => true,
                            'type' => 'text',
        ...
        foreach ($variantAttr as $varainAttr_value) {
            switch ($varainAttr_value['frontend_type']) {
                case 'checkbox':
                    $options = $optionsTable::find(["attribute_id='{$varainAttr_value['id']}'"]);
                    $attrOptions = [];
                    ...
                    array_push($varrAttrFields['formJson'], [
                        'attribute' => $varainAttr_value['label'],
                        'key' => $varainAttr_value['code'],
                        'field' => $varainAttr_value['frontend_type'],
                        'data' => [
                            "value" => [],
                            "values" => $attrOptions,
                            "required" => $varainAttr_value['is_required']
                        ]
                    ]);
                    break;
                case 'dropdown':
                ...
                case 'textarea':
                    array_push($varrAttrFields['formJson'], [
                        ...
                        'data' => [
                            "value" => "",
                            "placeholder" => $varainAttr_value['label'],
                            "required" => $varainAttr_value['is_required'] ,
                            "maxLength" => 1000
                        ]
                    ]);
                ...
                case 'textfield':
                    $fieldType = '';
                    if ($varainAttr_value['backend_type'] == 'varchar' || $varainAttr_value['backend_type'] == 'text') {
                        $fieldType = 'text';
                    } else {
                        $fieldType = 'number';
                    }
        ...
        foreach ($configAttributes as $key => $val) {
            $config_attributes[] = $val['code'];
        }
        $responseArray = [
            'product_form' => [$container_field],
            'config_attributes' => $variant_fields,
            'variant_attributes' => $config_attributes
        ];
        if (count($varrAttrFields['formJson']) > 0) {
