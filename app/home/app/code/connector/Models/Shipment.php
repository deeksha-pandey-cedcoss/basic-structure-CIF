...
class Shipment extends Base
{
    ...
    public function prepareShipmentData($shipmentData)
    {
        ...
        $itemIds = [];
        $userId = $this->di->getUser()->id;
        foreach ($shipmentData['shipmentData'] as $key => $value) {
        ...
        if (count($items) > 0) {
            ...
            foreach ($allItemDetails as $key => $singleItem) {
    ...
    public function createShipment($shipmentData)
    {
        ...
        if ($shipmentStatus) {
            ...
            $errors = '';
            foreach ($shipmentModel->getMessages() as $key => $value) {
                $errors .= ' ' . $value;
            }
    ...
    public function prepareHtmlOfShipmentForPdf($shipmentContent)
    {
        ...
        $itemTrHtml = '';
        $count = 1;
        foreach ($shipmentContent['shipment_items'] as $key => $value) {
            $trStyle = '';
            if (($count % 2) === 0) {
    ...
    public function getShipmentItems($shipmentItems, $orderId)
    {
        ...
        foreach ($shipmentItems as $key => $singleItem) {
            ...
            $total_qty += $singleItem['total_qty'];
        }
        return [
            'shipped_items' => $shippedItems,
            'total_qty_shipped' => $total_qty
        ];
    }

    public function getDiscountAndTaxAmount($singleItem)
    {
        $item_total_price = $singleItem['unit_price'] * $singleItem['total_qty'];
        $item_subtotal_price = $singleItem['unit_price'] * $singleItem['total_qty'];
        ...
        foreach ($discountCodes as $key => $singleDiscount) {
            if ($singleDiscount['type'] == 'percentage') {
                $totalDiscount += $singleDiscount['amount'] * $item_total_price;
            } elseif ($singleDiscount['type'] == 'fixed') {
                $totalDiscount += $singleDiscount['amount'];
            }
        }
        $item_subtotal_price = $item_subtotal_price - $totalDiscount;
        foreach ($taxLines as $key => $singleTax) {
            $totalTax += $singleTax['rate'] * $item_subtotal_price;
        }
        $item_subtotal_price = $item_subtotal_price + $totalTax;
        return [
