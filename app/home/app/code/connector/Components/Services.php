...
class Services extends \App\Core\Components\Base
{
    ...
    public function getServiceModel($service,$userId){
        ...
        /*$userService = $service['handler']::findFirst(['merchant_id' => $userId,'code' => $service['code']]);
        return $userService;*/
        $userService = $this->di
            ->getObjectManager()
        ...
        return $userService;
        //$service->setCollection($userService->getCollection());
        //return $service->setData($userService->getData());
    }
    /**
     * @param $code
     * @return mixed
    ...
    public function getUsableServices($type, $userId)
    {
        $services = $this->filterCollection($this->getAll(), ['type'=>$type]);
    }
