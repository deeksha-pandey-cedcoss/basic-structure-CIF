...
//        }
    ...
    private function getAttributes($cart) {
        $mongo = $this->di->getObjectManager()->create('\App\Core\Models\BaseMongo');
        $collection = $mongo->getCollectionForTable("shopify_cart_upload_".$this->_user_id);
        $uploadCart = $collection->findOne(['source_id' => $cart['source_id']]);
        if ( count($uploadCart) ) {
            $cart['shopifyUpload'] = $uploadCart;
        } else {
            $cart['shopifyUpload'] = [];
        }
        return $cart;
