...
//        $this->_shop_id = $this->di->getUser()->getShop()->id;
    ...
    public function getOrderByID($data) {
        ...
        // die;
        $res = $collection->findOne(["source_order_id" => (string)$data['order_id']],["typeMap" => ['root' => 'array', 'document' => 'array']]);
    ...
    {
        $mongo = $this->di->getObjectManager()->create('\App\Core\Models\BaseMongo');
        $collection = $mongo->setSource("order_container")->getPhpCollection();
...
//        }
    ...
    private function getAttributes($order) {
        $mongo = $this->di->getObjectManager()->create('\App\Core\Models\BaseMongo');
        $collection = $mongo->getCollectionForTable("shopify_order_upload_".$this->_user_id);
        $uploadOrder = $collection->findOne(['source_id' => $order['source_id']]);
        if ( count($uploadOrder) ) {
            $order['shopifyUpload'] = $uploadOrder;
        } else {
            $order['shopifyUpload'] = [];
        }
        return $order;
    ...
    private function buildAggregateQuery($params, $callType = 'getOrder')
    {
        $aggregation = [];
        $andQuery = [];
        $orQuery = [];
