...
class Product extends Base
{
    ...
    public function implodeArrays($array, $aliaz, $prefix)
    {
        ...
        if ($prefix == 'variant') {
            ...
            $marketplaceAttribute = ProductAttribute::find(["columns" => "code", "merchant_id='" . $merchantId . "' AND apply_on != 'base'"])
                ->toArray();
            foreach ($marketplaceAttribute as $key => $val) {
    ...
    public function getHeadersFromCsv($encodedFile)
    {
        ...
        $decoded_file = base64_decode($value);
        $fileType = substr($encodedFile['encoded_file'], strpos($encodedFile['encoded_file'], ':') + 1, strpos($encodedFile['encoded_file'], ';') - 4);
        $ext = 'xlsx';
    ...
    {
        return $this->di->getObjectManager()->get('importHelper')->getMappingSuggestions($sourceAttrs, $user_id, $source, $target);
    }
    ...
    public function exportProduct($columns)
    {
        ...
        $history_array['direction'] = 'export';
        $history_array['errors'] = '';
        $history_array['data_file'] = '';
        $history_array['no_of_product'] = '';
        try {
            ...
            $query1 = 'SELECT `pc`.id as `container_id`,' . $containerColumns . ', p.id as `variant_id`,' . $variantColumns . ' FROM `product_container_' . $user_id . '` as `pc` LEFT JOIN `product_' . $user_id . '` as `p` on (`p`.`product_id` = `pc`.`id`) WHERE pc.id IN (' . implode(',', $productsToExport) . ')';
            $query2 = "SELECT `pao`.`id`,`pa`.`code`,`pao`.`value`  FROM `product_attribute` as `pa` left join `product_attribute_option` as `pao` on `pao`.`attribute_id` = `pa`.`id` WHERE `merchant_id` = " . $user_id . " AND `frontend_type` in ('dropdown','radio','checkbox')";
            ...
            $path = BP . DS . 'var' . DS . 'exports' . DS . $user_id;
            if (!file_exists($path)) {
                $oldmask = umask(0);
            ...
            $history_array['data_file'] = $path . DS . $fileName . '.' . $columns['export_to'];
            $history_array['no_of_product'] = $containerCount;
            $this->importExportHistory($history_array);
            ...
            $history_array['errors'] = $e->getMessage();
            $this->importExportHistory($history_array);
    ...
    public function saveConfigAttributeMapping($data)
    {
        ...
        if (!file_exists($importFilePath) || !file_exists($mappingFilePath)) {
            ...
            fclose($handle);
            $finalarray = array_merge($varColumns, $contColumns);
            foreach ($configAttr as $k => $va) {
                $finalarray[] = $va['code'];
            }
    ...
    public function makeEntryInMarketplaceTable($productIds, $marketplace)
    {
    ...
    public function saveImages($imageUrl, $user_id, $count)
    {
        ...
        $fp = fopen($imgPath, 'w+');
        $pathToReturn = 'media' . DS . 'product' . DS . $user_id . DS . $imageName;
    ...
    public function updateImportHistory($history_index, $errors, $title)
    {
        $filePath = BP . DS . 'app' . DS . 'code' . DS . 'connector' . DS . 'etc' . DS . 'history.php';
        $data = [];
        if (file_exists($filePath)) {
            $data = require $filePath;
            if (isset($data[$history_index])) {
                if (empty($errors)) {
                    $data[$history_index]['success'] = $data[$history_index]['success'] + 1;
                } else {
                    $fvp = fopen($data[$history_index]['errors'], 'w');
    ...
    public function saveValueMapping($data)
    {
        ...
        if (!file_exists($importFilePath) || !file_exists($mappingFilePath)) {
            ...
            $mappingFileData = require $mappingFilePath;
            // $mappingFileData['value_mapping'] = $data['mapped_options'];
            foreach ($data['mapped_options'] as $key => $value) {
    ...
    public function getAndConditions($andCondition)
    {
        ...
        foreach ($conditions as $key => $value) {
            if (strpos($andCondition, $value) !== false) {
                ...
                //     trim($keyValue[0]) == 'short_description' ||
                //     trim($keyValue[0]) == 'long_description' ||
                //     trim($keyValue[0]) == 'vendor' ||
                //     trim($keyValue[0]) == 'product_type') {
                //     $prefix = 'details';
                // }
...
//             ];
            ...
            if (!$sendCount) {
                ...
                foreach ($response as $key => $productData) {
                    ...
                    // $productData = iterator_to_array($value);
                    //                    $marketplaceData = iterator_to_array($productData['fromItems']);
                    $storeId = false;
                        //                    foreach ($marketplaceData as $marketplaceKey => $marketplaceValue) {
                    //                        $mpData = iterator_to_array($marketplaceValue);
                    //                        $storeId = $mpData['store_id'];
                    //                    }
                    $variantAttrs = [];
...
//                ],
    ...
    public function getConfigurableAttributes($userId = false)
    {
        ...
        if ($response &&
            ...
            if (count($configurableAttributes)) {
                $configAtrributes = [
                    'user_id' => $userId,
