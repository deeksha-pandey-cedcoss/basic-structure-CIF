...
class Category extends BaseMongo
{
    ...
    public function extractCategory($dataForSave, $marketplace)
    {
    ...
    public function traverseChildren($childrenArr, $marketplace)
    {
        try {
            $childrenArrNew = [];
            $childrenChange = 0;
            foreach ($childrenArr as $key => $childrens) {
    ...
    public function savedCategory($data)
    {
        ...
        $savedData['custom_category_path'] = $savedData['custom_category_path'];
        return $savedData;
    }
