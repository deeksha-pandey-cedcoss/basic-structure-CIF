...
class Suggestor extends \App\Core\Components\Base
{
    ...
    public function getSuggestion($attributes, $user_id = false, $source = false, $target = 'connector')
    {
        ...
        $customerMappings = $this->getCustomerMappings($source, $target, $user_id); /* getCustomerMappings return all     the source attributes in key(target-attribute) => value(source-attribute) format which has been mapped by that customer before */
        $suggestions = [];
    ...
    public function getUploadMappingSuggestions($user_id = false, $targetAttrs, $sourceAttrs, $target = '')
    {
        $mappingSuggestions = [
            'basic_attributes' => [],
            'required_attributes' => [],
    ...
    public function submitMapping($mappedData, $user_id = false, $source = false, $target = 'connector')
    {
        $this->checkUserEntryExist($source, $target, $user_id); /* It is done to update the no. of mappings done between this source and target (user-wise)  */
        $systemMappings = $this->getSystemMappings($source, $target); /* getSystemMappings return all the source                                                                     attributes in key(target-attribute) =>                                                                      value(source-attribute) format whose                                                                        isSystem is true */
        $customerMappings = $this->getCustomerMappings($source, $target, $user_id); /* getCustomerMappings return all     the source attributes in key(target-attribute) => value(source-attribute) format which has been mapped by that customer before */
        $mappingData = [];
                ...
                if (isset($systemMappings[$targetAttribute])) {
                    ...
                    if (isset($customerMappings[$targetAttribute])) {
                        ...
                        if ($customerMappings[$targetAttribute] != $sourceAttr) {
                            ...
                            $updateCustomerSource = \App\Connector\Models\CustomerAttributeMapping::findFirst(["product_source='$source' AND target='$target' AND target_attr='{$targetAttribute}' AND source_attr='{$customerMappings[$targetAttribute]}' AND customer_id='{$user_id}'"]);
                        ...
                        $this->createNewMapping($source, $target, $sourceAttr, $targetAttribute);
                        $this->createNewCustomerMapping($source, $target, $sourceAttr, $targetAttribute, $user_id);
                    }
                }
            }
        }
    ...
    public function submitUploadMapping($mappingData, $user_id = false, $source = 'connector', $target = false)
    {
        $this->checkUserEntryExist($source, $target, $user_id); /* It is done to update the no. of mappings done between this source and target (user-wise)  */
        $systemMappings = $this->getSystemMappings($source, $target); /* getSystemMappings return all the source                                                                     attributes in key(target-attribute) =>                                                                      value(source-attribute) format whose                                                                        isSystem is true */
        $customerMappings = $this->getCustomerMappings($source, $target, $user_id); /* getCustomerMappings return all     the source attributes in key(target-attribute) => value(source-attribute) format which has been mapped by that customer before */
            ...
            if (isset($systemMappings[$targetAttr])) {
                ...
                if (isset($customerMappings[$targetAttr])) {
                    ...
                    if ($customerMappings[$targetAttr] != $sourceAttr) {
                        ...
                        $updateCustomerSource = \App\Connector\Models\CustomerAttributeMapping::findFirst(["product_source='$source' AND target='$target' AND target_attr='{$targetAttr}' AND source_attr='{$customerMappings[$targetAttr]}' AND customer_id='{$user_id}'"]);
                        $updateCustomerSource->source_attr = $sourceAttr;
                        $updateCustomerSource->save();
                    }
                } else {
                    /* If there is no such mapping before then new mapping is created */
                    $this->createNewMapping($source, $target, $sourceAttr, $targetAttr);
                    $this->createNewCustomerMapping($source, $target, $sourceAttr, $targetAttr, $user_id);
                }
            }
        }
    }

    public function checkUserEntryExist($source, $target, $user_id)
    {
        $checkEntry = \App\Connector\Models\CustomerAttributeMapping::findFirst(["product_source='$source' AND target='$target' AND customer_id='$user_id'"]); /* Checking whether this same user has done mapping between this same source and target before or not */
        if (!$checkEntry) {
    ...
    public function createNewMapping($source, $target, $sourceAttr, $targetAttribute)
    {
    ...
    public function checkReductionOfHits($source = '', $target = '', $sourceAttr = '', $targetAttr = '', $sourceAttrOption = '')
    {
    ...
    public function saveCustomerValueMapping($userId = '', $source = '', $target = '', $sourceAttr = '', $targetAttr = '', $sourceAttrOption = '', $targetAttrOption = '')
    {
    ...
    public function saveValueMapping($source = '', $target = '', $sourceAttr = '', $targetAttr = '', $sourceAttrOption = '', $targetAttrOption = '')
    {
    ...
    public function reduceHits($mappingEntries)
    {
    ...
    public function createNewCustomerMapping($source, $target, $sourceAttr, $targetAttribute, $user_id)
    {
        $mappingData = [];
        $mappingData['customer_id'] = $user_id;
        $mappingData['product_source'] = $source;
        $mappingData['target'] = $target;
        $mappingData['source_attr'] = $sourceAttr;
        $mappingData['target_attr'] = $targetAttribute;
    ...
    public function updateHitsOccurence($hitValue, $model)
    {
        $model->hits = $hitValue;
