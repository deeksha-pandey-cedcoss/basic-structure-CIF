...
class Helper extends Base
{
    ...
    public function getJwtToken($token, 
        ...
        return $jwt = JWT::encode($token, $privateKey, $algo);
    ...
    public function decodeToken($token, $check = true, $key = false)
    {
        try {
            if (count(explode('.', $token)) > 2) {
                ...
                $jwt = (array)JWT::decode($token, $key, array($header['alg']));
                ...
                if (!$check || $this->di->getTokenManager()->checkToken($jwt) || $jwt['role'] == 'app') {
                    ...
                    $result = ['success'=>false, 'code'=>'invalid_token', 'message'=>'Token is not valid anymore'];
                }
            } else {
                $result = ['success'=>false, 'code'=>'invalid_token', 'message'=>'Token is not valid anymore'];
            }
    ...
    public function addHandler($handler, $path, $overwrite = false)
    {
        ...
        $request = $this->curlRequest($this->di->getConfig()->rabbitmq_url . '/rmq/queue/addHandler', $data, false);
        return $request;
    }
    ...
    public function sendOtp($contactDetails) {
        ...
        $client = new \Twilio\Rest\Client($this->di->getConfig()->otp_details->account_number, $this->di->getConfig()->otp_details->access_token);
    ...
    public function generateChildAcl($subuser){
        ...
        $resources = \App\Core\Models\Resource::find();
        $components = [];
        foreach ($resources as $resoruce) {
            $components[$resoruce->module.'_'.$resoruce->controller][] = $resoruce->action;
        }
        foreach($components as $componentCode => $componentResources){
            $acl->addComponent($componentCode,$componentResources);
        }
        if(isset($subuser['resources']) && ($subuser['resources'] instanceof \Phalcon\Db\Adapter\MongoDB\Model\BSONArray || is_array($subuser['resources'])) ){
            foreach ($subuser['resources'] as $roleResource) {

                $resource = \App\Core\Models\Resource::findFirst([['_id'=>$roleResource]]);

                if ($resource) {
                    $acl->allow('child_' . $subuser['_id'], $resource->module . '_' . $resource->controller, $resource->action);
                }
            }
        }
    

        $directory = BP . DS . 'app' . DS . 'etc' . DS . 'security' . DS.'child';
        $file = $directory. DS. 'acl_' . $subuser['_id'] . '.data';
        $this->saveFile($directory, $file, serialize($acl),'w+');
    }
}
