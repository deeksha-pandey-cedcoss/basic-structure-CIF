...
class Schema extends \App\Core\Components\Base
{
    ...
    public function applyOnSingle($di, $moduleName, $currentVersion, $dbname = 'db', callable $callback)
    {
        ...
        if($di->getConfig()->databases->db->adapter == 'Mongo'){
                ...
                if ($dbVersion && isset($dbVersion['version'])) {
                    if ($currentVersion > $dbVersion['version']) {
                        ...
                        if ($result) {
                            ...
                            $collection->updateOne(["module" => $moduleName],['$set' =>["version" => $currentVersion ]]);
                        }
                    }
                } else {
            ...
            try {
                $dbVersion = $connection->query("Select version From setup_module where module = '{$moduleName}'")->fetch();
            ...
            try {
                ...
                if (isset($dbVersion['version'])) {
                    if ($currentVersion > $dbVersion['version']) {
                        ...
                        $result = call_user_func($callback, $dbConnection, $dbVersion['version']);
                        if ($result) {
                            //$connection->query("Update setup_module set version = '{$currentVersion}' where module = '{$moduleName}'");
                        }
                    }
                } else {
                    ...
                    $result = call_user_func($callback, $dbConnection, '');
                    if ($result) {
                        //$connection->query("Insert into setup_module (module, version) VALUES ('{$moduleName}', '{$currentVersion}')");
                    }
                }
