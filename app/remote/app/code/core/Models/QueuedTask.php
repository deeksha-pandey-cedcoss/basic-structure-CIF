...
class QueuedTask extends BaseMongo
{
    ...
    public function getQueuedTaskOfUser()
    {
        ...
        if (count($allUserFeeds)) {
            $allQueuedTask = [];
            foreach ($allUserFeeds as $feedKey => $feedValue) {
                ...
                $this->createNotification($feedValue, $feedValue->feed_message, $userId);
                }*/
                }
            }
            return ['success' => true, 'message' => 'All queued task', 'data' => $allQueuedTask];
        } else {
            return ['success' => true, 'code' => 'no_queued_task', 'message' => 'No queued task', 'data' => []];
        }
    }

    public function createNotification($completedFeed, $message, $userId)
    {
        $status = $completedFeed->delete();
        ...
        if (!$saveStatus) {
            ...
            $this->di->getLog()->logContent($message . ' ==> ' . $userId, \Phalcon\Logger::CRITICAL, 'notify-insert.log');
            $this->di->getLog()->logContent($errors, \Phalcon\Logger::CRITICAL, 'notify-insert.log');
        }
        return true;
    }
    ...
    public function getQueuedTaskResponse($data, $success)
    {
        ...
        if ($userFeed) {
            ...
            if (!$saveStatus) {
                $errors = implode(',', $notifications->getMessages());
                $this->di->getLog()->logContent($errors . ' ==> ' . $userId . ' ==> ' . $messageId, \Phalcon\Logger::CRITICAL, 'notify-insert.log');
            }
        }
    }
    ...
    public function insertNewQueuedTask($feedId, $userId = false, $message = 'Queued Task')
    {
        ...
        if (!$queuedTasks->save($feedData)) {
            ...
            $this->di->getLog()->logContent($errors, \Phalcon\Logger::CRITICAL, 'queued_task_insert.log');
            return false;
        }
